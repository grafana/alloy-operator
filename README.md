...
| Component        | Version |
|------------------|---------|
| Alloy Operator   | 0.5.10 |
| Alloy Helm chart | 1.10.0 |
| Alloy binary     | v1.17.0 |
...
The operator image is scanned for known vulnerabilities (CVEs) by the **CVE Scan** GitHub Action, which runs on
every pull request that touches `operator/**` or the [VEX ledger](.vex/), on pushes to `main`, and weekly (to
catch CVEs newly disclosed against the pinned base image). It uses two complementary scanners:

* **[Trivy](https://trivy.dev/)** matches package versions across the whole image (UBI RPMs and the bundled Go
  binary). It is the PR gate: it fails if a HIGH or CRITICAL CVE appears that is not justified in the VEX ledger.
* **[govulncheck](https://pkg.go.dev/golang.org/x/vuln/cmd/govulncheck)** (binary mode) adds symbol-level
  *reachability* for the bundled `helm-operator` Go binary — it reports which CVEs are actually linked into the
  binary, which Trivy cannot tell. It is advisory and is the signal that warns when a newly disclosed CVE
  becomes reachable.

Both publish results to the repository's **Security → Code scanning** tab. Scan locally with `make scan`
(Trivy) and `make reachability` (govulncheck); both fall back to a container image if the tool is not installed.
...
2. **Triage it** in the [VEX ledger](.vex/) if the finding does not affect this operator — add an OpenVEX
   `not_affected` statement with a written justification (see [`.vex/README.md`](.vex/README.md)). This keeps
   the scan green while leaving an auditable rationale for each accepted finding, rather than a bare ignore list.
